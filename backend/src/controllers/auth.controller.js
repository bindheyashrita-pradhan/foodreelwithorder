...
// Log out a user (FIXED TYPO)
function logoutUser(req, res) {
  res.clearCookie("token"); // Fixed: Changed "Token" (capital T) to "token" (lowercase t)
  res.status(200).json({ message: "User logged out successfully" });
}

// Register a new food partner business
async function registerFoodPartner(req, res) {
  ...
  res.cookie("token", token)
  res.status(201).json({ message: "Food Partner registered successfully", foodPartner: { _id: foodPartner._id, email: foodPartner.email, name: foodPartner.name, address: foodPartner.name, contactName: foodPartner.contactName, phone: foodPartner.phone } })
}

// Log in a food partner business
...
// Log out a food partner business
